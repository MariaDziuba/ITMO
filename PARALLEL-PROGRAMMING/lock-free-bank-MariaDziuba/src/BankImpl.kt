...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            val curAcc = account(index)
            if (curAcc.invokeOperation()) continue
            check(curAcc.amount - amount >= 0) { "Underflow" }
            val newAccAmount = Account(curAcc.amount - amount)
            if (accounts[index].compareAndSet(curAcc, newAccAmount)) return newAccAmount.amount
        }
    }
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
         ...
         */
        while (true) {
            val acc = account(index)
            if (op.completed) return null

            if (acc is AcquiredAccount) {
                if (acc.op == op) return acc
                if (!acc.invokeOperation()) continue
            }

            val acquiredAccount = AcquiredAccount(acc.amount, op)
            if (accounts[index].compareAndSet(acc, acquiredAccount)) {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val maxIdx = maxOf(toIndex, fromIndex)
            val minIdx = minOf(toIndex, fromIndex)

            val minAcquired = acquire(minIdx, this)
            val maxAcquired = acquire(maxIdx, this)

            if (maxAcquired != null) {
                if (minAcquired != null) {
                    val to = when (minIdx != toIndex) {
                        true -> maxAcquired
                        else -> minAcquired
                    }
                    val from = when (minIdx != fromIndex) {
                        true -> maxAcquired
                        else -> minAcquired
                    }
                    when {
